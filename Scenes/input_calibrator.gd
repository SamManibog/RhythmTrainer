...
func _ready():
	...
	inputSlider.applySliderSettings(-300, 300, 1).setOutputMod(func(input): return "Input will be received " + str(abs(input)) + "ms " + ("earlier" if input <= 0 else "later")).bindToSetting(settings.generalSettings, "inputCalibration")
	audioAdjust = settings.generalSettings.audioCalibration
