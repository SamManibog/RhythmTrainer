...
func _ready():
	...
	tutorialMenu.addTutorialData("Welcome!", "Welcome to Rhythm Trainer, an app where you can practice sightreading music. Tap anywere to begin, then tap out the rhythm on your screen. If you make a mistake, you can always use the button in the upper right corner to restart or go back a measure.", .6)
...
func highlightMeasure(measureNumber, lineNumber: int = 0):
	...
	if lineNum == -1: return
	var graphicLineNum = util.getNext(visibleLines, 0, func(line: graphicLine): return line.lineNumber == lineNum)
	if graphicLineNum == -1: return
	var line: lineGraphicData = graphicList[lineNum]
	var graphicLine: graphicLine = visibleLines[graphicLineNum]
